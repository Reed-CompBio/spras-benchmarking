...
def normalize_interactome(df: pandas.DataFrame) -> tuple[pandas.DataFrame, bool]:
    ...
    - Sorting the interactome by weight, then by interactome nodes
    ...
    """
    # sort by weight, then by (Interactor1 and Interactor2) to ensure deterministic sorting
    df_sorted = df.sort_values(by=["Weight", "Interactor1", "Interactor2"], ascending=True, ignore_index=True)
    ...
    min_nodes = df_sorted.loc[undirected_mask, ["Interactor1", "Interactor2"]].min(axis=1)
    max_nodes = df_sorted.loc[undirected_mask, ["Interactor1", "Interactor2"]].max(axis=1)

    # assigns the sorted Node1 and Node2 back into the df
    df_sorted.loc[undirected_mask, "Interactor1"] = min_nodes
    df_sorted.loc[undirected_mask, "Interactor2"] = max_nodes

    unique_edges_df = df_sorted.drop_duplicates(subset=["Interactor1", "Interactor2", "Direction"], keep="first", ignore_index=True)
