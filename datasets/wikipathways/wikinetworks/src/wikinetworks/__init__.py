            ...
            for anchor in list(anchors):
                ...
                #    anchor_to_node[anchor_graphid] = [node1, node2]
                    ...
                    node1DF = gpd.GeoDataFrame(
                        featureDFs["interactDF"][featureDFs["interactDF"].anchor == str(anchor_graphid)],
...
def getNodeLoc(features):
    ...
    nodeDF = pd.DataFrame(
        nodeData, columns=["textlabel", "graphid", "groupref", "centerX", "centerY", "width", "height", "color", "database", "databaseID"]
    )
    ...
    for j in range(0, len(edgesWithAnchors)):
        for i in range(0, len(edgesWithAnchors.anchor.iloc[j])):
            if edgesWithAnchors.anchor.iloc[j][i] in enzymes.keys():
                ...
                edgesWithAnchors2 = pd.concat([edgesWithAnchors2, pd.DataFrame([temp])], ignore_index=True)
...
def ckdnearest(gdA, gdB, k=1):
    ...
    gdf = pd.concat([gdA, gdB.loc[idx, gdB.columns != "geometry"].reset_index(), pd.Series(dist, name="dist")], axis=1)
...
def mapEndPoints(featureDFs):
    ...
    featureDFs["interactDF"]["matched_Node1"] = np.where(
        featureDFs["interactDF"]["node1_graphref"].isnull(), np.nan, featureDFs["interactDF"]["node1_graphref"]
    )
    ...
    featureDFs["interactDF"]["matched_Node2"] = np.where(
        featureDFs["interactDF"]["node2_graphref"].isnull(), np.nan, featureDFs["interactDF"]["node2_graphref"]
    )
        ...
        if not feature is "interactDF" and not feature is "groupDF":
            ...
            if len(groupIDs) >= 1:
                ...
                featureDFs["interactDF"]["matched_Node1_cat"] = np.where(
                    (featureDFs["interactDF"]["matched_Node1_dist"] >= distDF_node1["dist"]), feature, featureDFs["interactDF"]["matched_Node1_cat"]
                )
                ...
                featureDFs["interactDF"]["matched_Node2"] = np.where(
                    (featureDFs["interactDF"]["matched_Node2_dist"] >= distDF_node2["dist"]),
                ...
                featureDFs["interactDF"]["matched_Node2_cat"] = np.where(
                    (featureDFs["interactDF"]["matched_Node2_dist"] >= distDF_node2["dist"]), feature, featureDFs["interactDF"]["matched_Node2_cat"]
                )
...
def mapPointsToInteractions(featureDFs, featureList):
    ...
    cond1 = [
        any(
            item in matchRef_alias(list2[temp], featureDFs, featureList, anchor_graphids)
            for item in matchRef_alias(list1[temp], featureDFs, featureList, anchor_graphids)
        )
        or any(
            item in matchRef_alias(list2[temp], featureDFs, featureList, anchor_graphids)
            for item in matchRef_alias(list1[temp], featureDFs, featureList, anchor_graphids)
    ...
    featureDFs["interactDF"]["matched_Node1_dist"] = np.where(
        featureDFs["interactDF"]["matched_Node1_dist"] >= distDF_node1["dist"], distDF_node1["dist"], featureDFs["interactDF"]["matched_Node1_dist"]
    )

    featureDFs["interactDF"]["matched_Node1_cat"] = np.where(
        featureDFs["interactDF"]["matched_Node1_dist"] >= distDF_node1["dist"], "edge", featureDFs["interactDF"]["matched_Node1_cat"]
    )
...
def processInteractDF(featureDFs, featureList):
    ...
    featureDFs["interactDF"]["matched_Node1_textlabel"] = [
        matchRef(x, featureDFs, featureList, anchor_graphids) for x in featureDFs["interactDF"]["matched_Node1"].tolist()
    ]
    featureDFs["interactDF"]["matched_Node2_textlabel"] = [
        matchRef(x, featureDFs, featureList, anchor_graphids) for x in featureDFs["interactDF"]["matched_Node2"].tolist()
    ]
    ...
    featureDFs["interactDF"]["matched_Node1_dist"] = np.where(
        featureDFs["interactDF"]["matched_Node1_textlabel"].apply(replaceDist, textlabels=textlabels),
        0,
        featureDFs["interactDF"]["matched_Node1_dist"],
    )
    featureDFs["interactDF"]["matched_Node2_dist"] = np.where(
        featureDFs["interactDF"]["matched_Node2_textlabel"].apply(replaceDist, textlabels=textlabels),
        0,
        featureDFs["interactDF"]["matched_Node2_dist"],
    )

    featureDFs["interactDF"]["matched_Node1_cat"] = np.where(
        featureDFs["interactDF"]["matched_Node1_textlabel"].isnull(), "unmapped", featureDFs["interactDF"]["matched_Node1_cat"]
    )
    featureDFs["interactDF"]["matched_Node2_cat"] = np.where(
        featureDFs["interactDF"]["matched_Node2_textlabel"].isnull(), "unmapped", featureDFs["interactDF"]["matched_Node2_cat"]
    )
...
def makeGraph(featureDFs, featureList, reWire_Inferred_Groups=False):
        ...
        for n1 in node1:
            for n2 in node2:
                if not arrow is None:
                    ...
                    G.add_edge(
                        str(n1), str(n2), edgeID=str(edgeID), arrow=str(arrow), interaction=str(interaction), signal=str(signal), color=str(color)
                    )
                else:
    ...
    unlabeledNodes = set(
        featureDFs["interactDF"][featureDFs["interactDF"].matched_Node1 == featureDFs["interactDF"].matched_Node1_textlabel]["matched_Node1"].tolist()
        + featureDFs["interactDF"][featureDFs["interactDF"].matched_Node2 == featureDFs["interactDF"].matched_Node2_textlabel][
            "matched_Node2"
        ].tolist()
    )
    ...
    if reWire_Inferred_Groups:
        ...
        for group in node_grouprefs:
            if "unlabeled" in str(group):
                ...
                for x in groupNodes:
                    upstreamNodes = set(
                        flatten(featureDFs["interactDF"]["matched_Node1_textlabel"][featureDFs["interactDF"]["matched_Node2_textlabel"] == x])
                    )
                    downstreamNodes = set(
                        flatten(featureDFs["interactDF"]["matched_Node1_textlabel"][featureDFs["interactDF"]["matched_Node1_textlabel"] == x])
                    )
                    for y in groupNodes:
                        for node1, node2, arrow, edgeID, color in zip(
                            featureDFs["interactDF"]["matched_Node1_textlabel"].tolist(),
                                ...
                                for n2 in node2:
                                    if (n1 in upstreamNodes) & (x == n2) & (n1 != y):
                                        if not arrow is None:
                                            ...
                                            G.add_edge(
                                                str(n1), str(y), edgeID=str(edgeID), arrow="unknown", interaction="u", signal="u", color=str(color)
                                            )
