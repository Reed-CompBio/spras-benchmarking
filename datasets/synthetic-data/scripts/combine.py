...
if not (data_directory / "processed" / "interactomes" / "uniprot-combined-threshold-interactomes/").exists():
    (data_directory / "processed" / "interactomes" / "uniprot-combined-threshold-interactomes/").mkdir(exist_ok=True, parents=True)

thresholds = [1, 100, 200, 300, 400, 500, 600, 700, 800, 900]
pathway_dirs = [
    "Apoptosis_signaling",
    "B_cell_activation",
    "Beta3_adrenergic_rec",
    "Cadherin_signaling",
    "Hedgehog_signaling",
    "Insulin_IGF",
...
for threshold in thresholds:
    threshold_human_interactome = pd.read_csv(
        data_directory / "processed" / "interactomes" / "uniprot-threshold-interactomes" / f"uniprot_human_interactome_{threshold}.txt", sep="\t"
    )
    ...
    merged_df.to_csv(
        data_directory / "processed" / "interactomes" / "uniprot-combined-threshold-interactomes" / f"uniprot_combined_interactome_{threshold}.txt",
        sep="\t",
        index=False,
        header=False,
    )
...
# TODO the overlap analytics don't include the drop duplicates
...
overlap_info_df.to_csv(
    data_directory / "processed" / "interactomes" / "uniprot-combined-threshold-interactomes/overlap_info.csv", sep="\t", index=False
)
...
for threshold in thresholds:
    interactome_file = data_directory / "processed" / "interactomes" / "uniprot-threshold-interactomes" / f"uniprot_human_interactome_{threshold}.txt"
...
overlap_combined_info_df.to_csv(
    data_directory / "processed" / "interactomes" / "uniprot-combined-threshold-interactomes" / "overlap_combined_info.csv", sep="\t", index=False
)
