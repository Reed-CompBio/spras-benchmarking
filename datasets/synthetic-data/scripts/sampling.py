...
def sample_interactome(interactome_df: pandas.DataFrame, weight_mapping: OrderedDict[int, int], percentage: float):
    if percentage > 1: raise RuntimeError(f"Got a percentage above 1 ({percentage})?")
    if percentage == 1: return interactome_df
...
def main():
    ...
    for percentage in map(lambda x: x / 10, range(1, 10 + 1)):
        print(f"Sampling with {percentage:.2f}% of edges...")
        attempt_number = 1
...
def attempt_sample(pathway: str,
                   ...
                   percentage: float) -> Optional[list[tuple[str, str]]]:
