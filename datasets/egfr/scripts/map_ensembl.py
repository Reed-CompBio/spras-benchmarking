...
def main():
    ...
    gold_standard_nodes = (egfr_directory / "processed" / "uniprot" / "gold-standard-nodes.txt").read_text().splitlines()
    # and the prizes from `process_prizes.py`
    prizes = pandas.read_csv(egfr_directory / "processed" / "uniprot" / "input-nodes.txt", sep="\t")
