...
def main():
    ...
    gold_standard_nodes = (egfr_directory / "processed" / "gold-standard-nodes-uniprot.txt").read_text().splitlines()
    # and the prizes from `process_prizes.py`
    prizes = pandas.read_csv(egfr_directory / "processed" / "input-nodes-uniprot.txt", sep="\t")
    ...
    idmapped_input_nodes_df.to_csv(egfr_directory / "processed" / "input-nodes.txt", sep="\t", index=False)
